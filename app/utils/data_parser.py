...
class DataParser:
    def __init__(self, raw_data: dict):
        ...
        logger.info("DataParser inicializado com dados brutos")
        ...
        try:
            for cart_id, orders in self.raw_data.items():
                for order in orders:
                    ...
                    total_orders += 1
            logger.info(f"Parse concluído: {total_orders} pedidos extraídos")
        except Exception as e:
            logger.exception(f"Erro ao parsear pedidos: {e}")
            raise
        return orders_list