...
def test_sku_nicho_crud_operations():
    ...
    # Create
    response = client.post("/sku_nicho/inserir", params={"sku": test_sku, "nicho": "CRUD Test"})
    assert response.status_code == 200
    ...
    # Update
    response = client.put("/sku_nicho/atualizar", params={"sku": test_sku, "novo_nicho": "Updated CRUD Test"})
    assert response.status_code == 200
...
def test_websocket_route_registration():
    ...
    websocket_routes = [route for route in app.routes if hasattr(route, 'path') and route.path == "/ws/relatorio_diario"]
